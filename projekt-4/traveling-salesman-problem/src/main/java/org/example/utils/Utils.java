...
public class Utils {
    ...
    public static Map<Integer, List<Integer>> generateCandidateVertices(int[][] matrix, int k) {
        int n = matrix.length;
        ...
        for (int i = 0; i < n; i++) {
            ...
            int finalI = i;
            neighbors.sort(Comparator.comparingInt(j -> matrix[finalI][j]));

            candidateEdges.put(i, neighbors.subList(0, Math.min(k, neighbors.size())));
        }
